...
import app from "../../../app";
import { setupTestEnvironment, teardownTestEnvironment, createTestUser, testContext, makeApiRequest, makeAuthenticatedApiRequest, DEFAULT_PASSWORD_STRONG, DEFAULT_PASSWORD_WEAK, } from "../../../test-utils";
describe("Authentication Service", () => {
    ...
    describe("Service Registration", () => {
        it("should register authentication service with JWT strategy", async () => {
            ...
            assert.ok(authenticationService.strategies, "Service should have strategies");
            assert.ok(authenticationService.strategies.jwt, "JWT strategy should be registered");
        });
        it("should register authentication service with Local strategy", async () => {
            ...
            const authenticationService = service;
            assert.ok(authenticationService.strategies, "Service should have strategies");
            assert.ok(authenticationService.strategies.local, "Local strategy should be registered");
        });
    ...
    describe("Authentication with User Context", () => {
        ...
        before(async () => {
            ...
            testUser = await createTestUser(userService, `authtest_${Date.now()}@example.com`, DEFAULT_PASSWORD_STRONG);
        });
        describe("Local Strategy Authentication", () => {
            it("should authenticate user with valid email and password", async () => {
                ...
                assert.ok(response.data?.authentication, "Should return authentication data");
            });
        ...
        describe("JWT Strategy Authentication", () => {
            ...
            it("should authenticate with JWT token", async () => {
                ...
                const jwtAuthResponse = await makeAuthenticatedApiRequest(port, "/authentication", jwtToken, {
                    strategy: "jwt",
                    accessToken: jwtToken,
                });
                assert.strictEqual(jwtAuthResponse.status, 201);
                assert.ok(jwtAuthResponse.data?.authentication, "Should return authentication data");
            });
            it("should reject authentication with invalid JWT token", async () => {
                const jwtToken = "invalid.jwt.token";
                const response = await makeAuthenticatedApiRequest(port, "/authentication", jwtToken, {
                    strategy: "jwt",
                    accessToken: jwtToken,
                });
                assert.strictEqual(response.status, 401);
    ...
    describe("Error Handling", () => {
        it("should handle malformed request body", async () => {
            const response = await makeApiRequest(port, "/authentication", "invalid json");
            // Should return appropriate error status or network error
            if (response.errors) {
                assert.ok(response.errors.length > 0, "Should have error messages for malformed JSON");
            }
            else {
                assert.ok(response.status && response.status >= 400, "Should return error status for malformed JSON");
            }
        });
