...
import app from "../../../app";
import { setupTestEnvironment, teardownTestEnvironment, createTestUser, createTestSession, testContext, DEFAULT_PASSWORD_STRONG, } from "../../../test-utils";
describe("Sessions Service", () => {
    ...
    before(async () => {
        ...
        const viewerUser = await createTestUser(userService, "viewer-test@example.com", DEFAULT_PASSWORD_STRONG, { id: "viewer-test-user-id", roles: ["viewer"] });
        const broadcasterUser = await createTestUser(userService, "broadcaster-test@example.com", DEFAULT_PASSWORD_STRONG, { id: "broadcaster-test-user-id", roles: ["broadcaster"] });
    ...
    describe("Viewer Sessions for Public Broadcast", () => {
        ...
        it("should not allow unauthenticated users to patch a session", async () => {
            const response = await makeApiRequest(port, `/sessions/${publicBroadcastSession.id}`, { title: "New Title" }, "PATCH");
            assert.strictEqual(response.status, 401);
        });
